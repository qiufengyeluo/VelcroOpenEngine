...
fn hash128_to_64(x: &U128) -> u64 {
    let mut _a: u64 = (u128_low64(x) ^ u128_high64(x)).wrapping_mul(KMUL);
    _a ^= _a >> 47;
    let mut _b: u64 = (u128_high64(x) ^ _a).wrapping_mul(KMUL);
    _b ^= _b >> 46;
    _b  = _b * KMUL;
...
fn hash_len_16(u: u64, v: u64, mul: u64) -> u64 {
    let mut _a: u64 = (u ^ v).wrapping_mul(mul);
    _a ^= _a >> 47;
    let mut _b = (v ^ _a).wrapping_mul(mul);
    _b ^= _b >> 47;
    _b = _b.wrapping_mul(mul);
...
fn hash_len_0_to_16(s: &[u8], len: usize) -> u64 {
    if len >= 8 {
        ...
        let mut _c: u64 = rotate64(_b, 37).wrapping_mul(_mul).wrapping_add(_a);
        let mut _d: u64 = rotate64(_a, 25).wrapping_add(_b).wrapping_mul(_mul);

        return hash_len_16(_c, _d, _mul);
    }

    if len >= 4 {
        let mut _mul = K2.wrapping_add(len as u64).wrapping_mul(2);
        let mut _a: u64 = fetch32(s) as u64;
        return hash_len_16(len as u64 + (_a << 3), fetch32(&s[len - 4..]) as u64, _mul);
    }

    if len > 0 {
        let mut _a = s[0];
        let mut _b: u8 = s[len >> 1];
        let mut _c: u8 = s[len -1];
        let mut _y = _a as u32 + ((_b as u32) << 8);
        let mut _z: u32 = (len as u32) + ((_c as u32) << 2);
        return shift_mix((_y as u64).wrapping_mul(K2) ^ (_z as u64).wrapping_mul(K0)).wrapping_mul(K2);
    }
...
fn hash_len_33_to_64(s: &[u8], len: usize) -> u64 {
    ...
    _b = shift_mix(_z.wrapping_add(_a).wrapping_mul(_mul).wrapping_add(_d).wrapping_add(_h)).wrapping_mul(_mul);
...
fn weak_hash_len32_with_seeds_bytes(s: &[u8], a: u64,  b: u64) -> U128 {
    return weak_hash_len32_with_seeds(fetch64(&s[0..]), 
                                    fetch64(&s[8..]), 
                                    fetch64(&s[16..]), 
                                    fetch64(&s[24..]), 
                                    a, 
...
pub fn city_hash64(mut s: &[u8], len: usize) -> u64 {
    ...
    let mut _z: u64 = hash_len_16_2(fetch64(&s[len - 48..]).wrapping_add(len as u64), fetch64(&s[len -24..]));
    ...
    loop {
        _x = rotate64(_x.wrapping_add(_y).wrapping_add(_v.0).wrapping_add(fetch64(&s[8..])), 37).wrapping_mul(K1);
        ...
        _v = weak_hash_len32_with_seeds_bytes(&s[0..], _v.1.wrapping_mul(K1), _x.wrapping_add(_w.1));
        _w = weak_hash_len32_with_seeds_bytes(&s[32..], _z.wrapping_add(_w.1), _y.wrapping_add(fetch64(&s[16..])));
    ...
    return hash_len_16_2(hash_len_16_2(_v.0, _w.0).wrapping_add(_y).wrapping_mul(K1).wrapping_add(_z), 
...
fn city_murmur(mut s: &[u8], len: usize, seed: U128)-> U128 {
    ...
    if _l <= 0 {
        ...
        _d = hash_len_16_2(_b.wrapping_add(len as u64), _c.wrapping_add(fetch64(&s[len - 16..])));
...
fn city_hash128_with_seed(mut s: &[u8], mut len: usize, seed: U128) -> U128 {
    ...
    _w.0 = rotate64(_y.wrapping_add(_z), 35).wrapping_mul(K1).wrapping_add(_x);
    ...
    loop {
        ...
        _y = _y.wrapping_add(_v.0).wrapping_add(fetch64(&s[40..]));
        _z = rotate64(_z.wrapping_add(_w.0), 33).wrapping_mul(K1);
        _v = weak_hash_len32_with_seeds_bytes(&s[8..], _v.1.wrapping_mul(K1), _x.wrapping_add(_w.0));
        _w = weak_hash_len32_with_seeds_bytes(&s[32..], _z.wrapping_add(_w.1), _y.wrapping_add(fetch64(&s[16..])));
        ...
        _v = weak_hash_len32_with_seeds_bytes(&s[0..],  _v.1.wrapping_mul(K1), _x.wrapping_add(_w.0));
        _w = weak_hash_len32_with_seeds_bytes(&s[32..], _z.wrapping_add(_w.1), _y.wrapping_add(fetch64(&s[16..])));
    ...
    return U128(hash_len_16_2(_x.wrapping_add(_v.1), _w.1).wrapping_add(_y), 
...
pub fn city_hash128(s: &[u8], len: usize) -> U128 {
    if len >= 16 {
        return city_hash128_with_seed(&s[16..], len - 16, U128(fetch64(&s[0..]), fetch64(&s[8..]).wrapping_add(K0)));
    }
